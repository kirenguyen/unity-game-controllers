...
for i in $( ls rosbag_inputs/); do
	...
	echo "$interFileName"
	echo "~~~"
	# play the rosbag and affdex...session_number = sys.argv[2] interaction_log = sys.argv[3]
	#gnome-terminal --geometry 40x120+0+0 --title ">>>AFFDEX SCRIPT<<<" -x bash -c "sh start_rosbag_analysis.sh $rosbagName $sessNumber $interFileName"
	#####################
...
python3 extract_affdex_data.py $rosbagName $sessNumber $interFileName # start the analysis
